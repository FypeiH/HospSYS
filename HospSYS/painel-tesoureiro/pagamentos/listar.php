...
$txtpesquisar = '%'.@$_POST['txtpesquisar'].'%';
  $res = $pdo->query("SELECT * from pagamentos where funcionario LIKE '$txtpesquisar' and tesoureiro = '$id_utilizador' and (data >= '$dataInicial' and data <= '$dataFinal') order by id asc");
...
for ($i=0; $i < count($dados); $i++) { 
  ...
  $res_excluir = $pdo->query("SELECT * from utilizadores where id = '$id_utilizador'");
