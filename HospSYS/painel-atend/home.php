...
$resultado_pend = $pdo->query("SELECT * from consultas where estado_pagamento != 'Sim'");
...
$resultado_hoje = $pdo->query("SELECT * from consultas where data = curDate() and estado_pagamento = 'Sim'");
$dados_hoje = $resultado_hoje->fetchAll(PDO::FETCH_ASSOC);
$valor_hoje = count($dados_hoje);

//Contar as consultas a aguardar

$resultado_aguar = $pdo->query("SELECT * from consultas where data = curDate() and status = 'a Aguardar'");
...
<table class="table table-sm mt-3">
  ...
  $res = $pdo->query("SELECT * from consultas where data = curDate() and estado_pagamento = 'Não' and status = 'a Aguardar' order by hora asc LIMIT 6");
