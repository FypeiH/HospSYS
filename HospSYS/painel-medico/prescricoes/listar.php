...
if($txtpesquisar == ''){
	...
	$res = $pdo->query("SELECT * from consultas where data = curDate() and medico = '$id_med' and prescricao = 'Sim' order by hora asc LIMIT $limite, $itens_por_pagina");

}else{

	$txtpesquisar = @$_POST['txtpesquisar'];
	$res = $pdo->query("SELECT * from consultas where data = '$txtpesquisar' and medico = '$id_med' and prescricao = 'Sim' order by hora asc");
