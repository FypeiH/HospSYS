...
if($txtpesquisar == ''){
	...
	$res = $pdo->query("SELECT * from consultas where data = curDate() and medico = '$id_med' and receita = 'Sim' order by hora asc LIMIT $limite, $itens_por_pagina");

}else{

	$txtpesquisar = @$_POST['txtpesquisar'];
	$res = $pdo->query("SELECT * from consultas where data = '$txtpesquisar' and medico = '$id_med' and receita = 'Sim' order by hora asc");
...
for ($i=0; $i < count($dados); $i++) { 
  ...
  <td>
    <a target="_blank" href="rel/rel_receita_class.php?id='.$id.'"><i class="fas fa-file-medical-alt text-info mr-1" title="Gerar Receita"></i></a>
  </td>
